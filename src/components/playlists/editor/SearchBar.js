...
function ScrollableSection({ title, songs = [], handlePlaySong }) {
  const { currentSongs, addSongs } = usePlaylistEditStore();
  const [addedSongs, setAddedSongs] = useState(new Set());

  // Update added songs when playlist changes
  useEffect(() => {
    const addedIds = new Set(currentSongs.map(song => song.id));
    setAddedSongs(addedIds);
  }, [currentSongs]);

  const handleAddToPlaylist = (song) => {
    // Format the song to match the playlist item structure
    const playlistSong = {
      id: `new-${Date.now()}`,
      video_id: song.id,
      title: song.title,
    ...
    addSongs([playlistSong]);
    setAddedSongs(prev => new Set(prev).add(playlistSong.id));
